...
export LLM_INTERPRETATION_ENABLED="${LLM_INTERPRETATION_ENABLED:-true}"
export LLM_PROVIDER="${LLM_PROVIDER:-mock}"
export LLM_MODEL="${LLM_MODEL:-deterministic-marketing-interpreter}"

echo "Running daily marketing report workflow with LLM interpretation enabled."
echo "LLM provider: $LLM_PROVIDER"
echo "LLM model: $LLM_MODEL"
echo "No external LLM API key is needed when LLM_PROVIDER=mock."
echo

uv run python -m marketing_ops_agent.workflows.daily_marketing_report

echo
echo "Workflow with LLM interpretation finished."

latest_report="$(find reports -maxdepth 1 -type f -name '*.md' -print 2>/dev/null | sort | tail -n 1 || true)"
if [[ -n "$latest_report" ]]; then
  echo "Latest report: $latest_report"
  echo "Inspect it with: sed -n '1,260p' '$latest_report'"
else
